...
namespace VLB
{
    ...
    public class DynamicOcclusionDepthBuffer : DynamicOcclusionAbstractBase
    {
        ...
        protected override string GetShaderKeyword() { return ShaderKeywords.OcclusionDepthTexture; }
        ...
        void UpdateDepthCameraPropertiesAccordingToBeam()
        {
            ...
            var localRot = m_Master.beamInternalLocalRotation;
            if(Mathf.Sign(scale.z) < 0f) localRot *= Quaternion.Euler(0f, 180f, 0f);
            m_DepthCamera.transform.localRotation = localRot;

            if (!Mathf.Approximately(scale.y * scale.z, 0))
            {
                const float kMinNearClipPlane = 0.1f; // should be the same than in shader
                m_DepthCamera.nearClipPlane = Mathf.Max(apexDist, kMinNearClipPlane) * Mathf.Abs(scale.z);
                m_DepthCamera.farClipPlane = (m_Master.maxGeometryDistance + apexDist) * Mathf.Abs(scale.z);
                m_DepthCamera.aspect = Mathf.Abs(scale.x / scale.y);
                m_DepthCamera.fieldOfView = Mathf.Atan2(m_Master.coneRadiusEnd * Mathf.Abs(scale.y), m_DepthCamera.farClipPlane) * Mathf.Rad2Deg * 2.0f;
            }
        }
        ...
        void InstantiateOrActivateDepthCamera()
        {
            ...
            else
            {
                ...
                gameObject.ForeachComponentsOnlyInChildren<Camera>(cam => DestroyImmediate(cam.gameObject), true);
        ...
        protected override void OnModifyMaterialCallback(MaterialModifier.Interface owner)
        {
            ...
            var scale = m_Master.lossyScale;
            owner.SetMaterialProp(ShaderProperties.DynamicOcclusionDepthProps, new Vector3(Mathf.Sign(scale.x) * Mathf.Sign(scale.z), Mathf.Sign(scale.y), fadeDistanceToSurface));;
        }
